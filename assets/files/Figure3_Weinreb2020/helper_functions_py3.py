...
def adjacency_matrix(arg):
	if type(arg)==str: # path to a SPRING plot
		springdir = arg
		n = len(np.load(springdir+'/cell_filter.npy'))
		A = scipy.sparse.lil_matrix((n,n))
		for l in open(springdir+'/edges.csv').read().split('\n')[:-1]:
			l = l.split(';')
			A[int(l[0]),int(l[1])] = 1
			A[int(l[1]),int(l[0])] = 1
		return A
	
	elif type(arg)==list: # list of edges
		pass

	else: # knn_edges array from approx nearest neighbors
		knn_graph = arg
		n = knn_graph.shape[0]
		A = scipy.sparse.lil_matrix((n,n))
		for i in range(n):
...
def filter_dict(d, filt):
	for k,v in d.items():
		if k != 'meta' and k != 'filters':
			if len(v.shape) == 1:
				d[k] = v[filt]
			else:
				d[k] = v[filt,:]
	d['filters'].append(filt)
	return d

def combine_filters(filters):
	final = np.arange(len(filters[0]))
	for f in filters:
		final = final[f]
	return final
...
def get_knn_graph2(X, k=5, dist_metric='euclidean',approx=True):
	t00 = time.time()
	if approx:
		try:
...
def get_PCA_sparseInput(E, base_ix=[], numpc=50, method='sparse', normalize=False):
...
def run_all_spring_1_6(E, gene_list, sample_name, save_dir = './', base_ix = [], normalize = True,
				   ...
				   precomputed_pca = [], gene_filter = [], custom_colors = {},
				   exclude_corr_genes_list = [], exclude_corr_genes_minCorr = 0.2, dist_metric = 'euclidean', use_approxnn=True,
