...
def _add_CytoTRACE_results(adata, path):

    cytotrace_df = pd.read_csv(path, index_col=0)
    ...
    if n_dim > 1:
        if data.shape[0] == n_cells:
            for col in data.columns.tolist():
                if not col in adata.obs_keys():
                    add_to = "obs"
                    adata.obs[col] = data[col].values
        elif not name in adata.uns_keys():
            adata.uns[name] = data
            add_to = "uns"
...
# -----------------------------------------------------------------------------
def add_extended_files(adata, return_obj=False, write_h5ad=False):
